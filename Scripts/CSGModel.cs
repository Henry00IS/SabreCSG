				...
				if (activeTool == null || activeTool.BrushesHandleDrawing)
				{
					...
					for (int brushIndex = 0; brushIndex < brushes.Count; brushIndex++) 
					{
						...
						for (int i = 0; i < polygons.Length; i++)
						{
							...
							{
								Vector3 position = brushTransform.TransformPoint((Vector3)polygon.Vertices[j].Position);
								GL.Vertex(position);

								if (j < polygon.Vertices.Length - 1)
								{
									Vector3 position2 = brushTransform.TransformPoint((Vector3)polygon.Vertices[j + 1].Position);
									GL.Vertex(position2);
								}
								else
								{
									Vector3 position2 = brushTransform.TransformPoint((Vector3)polygon.Vertices[0].Position);
									GL.Vertex(position2);
								}
							}
						}
